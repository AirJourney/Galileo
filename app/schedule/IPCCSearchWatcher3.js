...
class IPCCSearchWatcher extends Subscription {
  ...
  async subscribe() {
    ...
    const result = await ctx.curl(
      "http://139.196.180.135:9001/support/getsellfight",
      // "http://127.0.0.1:9001/support/getsegmentlist",
      {
        ...
        dataType: "json",
        headers: {},
        data: { GDS: "travelport", GDSBooking: "Galileo" },
    ...
    if (
      ...
      const chunkSize = Math.ceil(result.data.content.length / 3);
    ...
    const callSegmentRT = async function (i, y, pccSeg) {
      ...
      ctx.logger.info(
        `cruise result: RT ${pccSeg.depart}-${pccSeg.arrival} ${forwardDay}-${backwardDay} request`
      );
      const airlineLength = await ctx.service.segment.cruiseGalileo(
        leg,
        pccSeg.IPCC,
        pccSeg.group
      );
      ctx.logger.info(
        `cruise result: RT ${pccSeg.depart}-${pccSeg.arrival} ${forwardDay}-${backwardDay} ${airlineLength}`
      );
    };
    ...
    const callSegmentOW = async function (i, pccSeg) {
      ...
      ctx.logger.info(
        `cruise result: OW ${pccSeg.depart}-${pccSeg.arrival} ${forwardDay} request`
      );
      ...
      ctx.logger.info(
        `cruise result: OW ${pccSeg.depart}-${pccSeg.arrival} ${forwardDay} ${airlineLength}`
      );
    };
    ...
    try {
      ...
      ctx.logger.error("Schedule Error", ex);
    }
  }
}
